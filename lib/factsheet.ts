...
// v5: 答えの具体性ルール（対象・目的語の省略禁止）
// v6: 散文（箇条書きなし）対応。一文・一主張分解＋総仕上げの網羅パス
// v7: 欄外・周辺情報の除外（学年・学期の対応、出典・ページ番号・脚注等。写真の隅の注記が出題される事故対策）
// v8: v7の学年除外がプロンプト層のみで再発したため、学年・学期の機械ガードを追加（確定的に落とす）
export const FACTSHEET_VERSION = 8
